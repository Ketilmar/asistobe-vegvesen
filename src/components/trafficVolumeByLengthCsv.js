...
const TrafficVolumeByLengthCsv = (data) => {
    ...
    const getValues = (data) => {
        if(typeof data !== 'object'){
          // Replace all commas with hyphen as comma is used to separate values in our csv
        ...
        // This is used to identify where to slice the 'rowItems' array into separate arrays (rows)
      ...
      if (fs.existsSync(path)){
        const csv = [[], ...rows ].join('\r\n');
        FileWriter(path, csv,'ID ' +idInfo[0] + ' - Write without header');
      }
      else {
        // Headers must be defined manually
        let manualHeaders = ['id','name','county','municipality', 'lat','lon','From', 'To', 'total-volume','Total-coverage', 'byLengthRange:..-5.6', 'LengthRange:5.6-..','LengthRange:5.6-7.6','LengthRange:7.6-12.5','LengthRange:12.5-16','LengthRange:16-24','LengthRange:24-..']

        // join header and body, and break into separate rows
        const csv = [manualHeaders, ...rows].join('\r\n');

        FileWriter(path, csv, 'ID ' +idInfo[0] + ' - Write with header');
      }
}
